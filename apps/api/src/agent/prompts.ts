...
7b. TAP-TO-EDIT CONTRACT — hard gate (automated hygiene runs AFTER design polish).
   ...
   - icon:   <parent>-icon, <parent>-chevron, <parent>-close.
     EVERY <EditableIcon> MUST have a testID. Even the tab bar icons in
     app/(tabs)/_layout.tsx must be <EditableIcon> with a testID (e.g.
     "home-tab-icon" / "settings-tab-icon"). For any inline icon in a screen,
     use the parent element's testID + -icon, -chevron, or -close suffix.
